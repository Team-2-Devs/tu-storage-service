...
public sealed class PresignPutUrlTests
{
  // Happy path
  [Fact]
  public async Task HandleAsync_ValidCommand_SucceedsAndCallsPresigner()
  {
    ...
    {
      UrlToReturn = "http://local/put",
      ExpiresAtToReturn = DateTimeOffset.Parse("2030-01-01T00:00:00Z")
    };
    var sut = new PresignPutUrl(presigner);
    ...
    var success = result as PresignPutUrlResult.Success;
    success.Should().NotBeNull();
    success!.Url.Should().Be("http://local/put");
    success.ExpiresAt.Should().Be(DateTimeOffset.Parse("2030-01-01T00:00:00Z"));

    presigner.LastMethod.Should().Be("PUT");
    ...
    presigner.LastContentType!.Value.Should().Be("image/jpeg"); // PUT requires ContentType
  }
  ...
  public async Task HandleAsync_InvalidKey_ReturnsInvalid_AndSkipsPresigner()
  {
    ...
    var invalid = result as PresignPutUrlResult.Invalid;
  ...
  [Fact]
  public async Task HandleAsync_InvalidContentType_ReturnsInvalid_AndSkipsPresigner()
  {
    var presigner = new FakeObjectStoragePresigner();
    var sut = new PresignPutUrl(presigner);
